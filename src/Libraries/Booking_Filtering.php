...
class Booking_Filtering {
    ...
    /**
     * Query user's booking
     * @var int
     */
    public $user_id;

    /**
     * Query author's booking
     * @var int
     */
    public $author_id;
    ...
    public function get_result()
    {
        ...
        $booking_status = array_keys(listar_booking_status());
        if(isset($this->request['post_status']) && in_array($this->request['post_status'], $booking_status)) {
            $booking_status = [$this->request['post_status']];
        }

        $args = [
            'post_status' => $booking_status,
            'post_type' => Listar::$post_type.'_booking',
            'paged' =>  $this->paged,
